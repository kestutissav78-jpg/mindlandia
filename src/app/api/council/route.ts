...
export async function POST(req: Request) {
  try {
    ...
    const gpt = await askAgent(
      openai,
      `You are GPT Strategist in MindLandia.
Always reply in ${replyLanguage}.
Give concise, practical strategic advice for startups, apps, product decisions and business planning.
...
Challenge the strategy, find weak logic, risks, missing assumptions, over-optimism, and practical blockers. Be direct but constructive.`,
    ...
    const decision = await askAgent(
      openai,
      `You are MindLandia Decision Maker.
Always reply in ${replyLanguage}.
Your output must be short, direct and mobile-friendly. Do not write long reports. Do not include long explanations.`,
